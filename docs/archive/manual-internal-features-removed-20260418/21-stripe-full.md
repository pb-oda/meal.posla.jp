...
::: tip 2026-04-09 α-1 構成
旧 standard/pro/enterprise 構成は廃止。現行は **基本料金 ¥20,000/月 + 追加店舗 ¥17,000/月 + 本部一括配信アドオン ¥3,000/月** の単一プラン+アドオン構成。詳細は `tenant/21-stripe-full.md` の 21.2 を参照。
:::

### 21.2.1 テナント側の申込み手順

1. POSLAのランディングページまたはオーナーダッシュボードの「契約・プラン」タブを開きます
2. 店舗数を入力し、本部一括配信アドオンの有無を選択します
3. 「30日間無料で始める」ボタンをクリックします
4. Stripe Checkoutの決済ページにリダイレクトされます
5. クレジットカード情報を入力します（カード登録必須・トライアル中は課金なし）
6. 認証完了後、自動的にPOSLAに戻り、トライアルが開始されます（subscription_status=trialing）
7. 31日目から自動で月額課金が開始されます

### 21.2.2 構成変更・解約

α-1 構成では「プラン変更」は存在しない。代わりに以下の3種類の構成変更が発生する。

**店舗追加：**
1. オーナーダッシュボード > 「店舗管理」 > 「+ 店舗を追加」
2. POSLA が Stripe Subscription の `additional_store` Price の quantity を +1
3. 差額（¥17,000/店舗）が日割り計算で即時課金

**店舗削除：**
1. オーナーダッシュボード > 「店舗管理」 > 削除
2. POSLA が `additional_store` Price の quantity を -1
3. 次回請求時に減額（日割り返金なし）

**本部一括配信アドオンの追加・解除：**
1. オーナーダッシュボード > 「決済設定」タブ > 「本部一括配信アドオン」セクション
2. POSLA が `hq_broadcast` Price を Stripe Subscription に追加または削除
3. POSLA 側で `tenants.hq_menu_broadcast = 1 / 0` に更新
4. 追加：owner-dashboard に「本部メニュー」タブが即時出現
5. 解除：「本部メニュー」タブが非表示になり、店舗側 dashboard 「店舗メニュー」タブで本部マスタが編集可能に戻る
...
- 契約状態（trialing / active / past_due / canceled）
- 店舗数（基本料金1 + 追加店舗）
- 本部一括配信アドオンの有無
- 月額合計（次回請求金額）
- 次回請求日
...
**ステップ1：Stripe Dashboardで商品を作成（α-1 構成）**
1. [Stripe Dashboard](https://dashboard.stripe.com/) にログイン
2. 「商品」→「商品を追加」
3. 3つの商品を作成：
   - **POSLA 基本料金** — 月額 ¥20,000（quantity=1 固定。trial_period_days=30 を Checkout 生成時に渡す）
   - **POSLA 追加店舗** — 月額 ¥17,000（quantity = 店舗数 - 1）
   - **POSLA 本部一括配信アドオン** — 月額 ¥3,000（quantity = 店舗数。アドオン契約時のみ）
...
| 問題 | 原因 | 対処 |
|------|------|------|
| サブスクリプション開始後に契約状態が反映されない | Webhookが届いていない | Stripe Dashboardでイベントログを確認。再送信 |
| 店舗追加・本部配信アドオン契約後もStripeに反映されない | `customer.subscription.updated` Webhook失敗 | Webhookログ確認。`subscription_events` テーブルでイベント受信状況を確認 |
