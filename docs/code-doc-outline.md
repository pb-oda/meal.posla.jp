...
### 1.6 決済アーキテクチャ
- 3つの決済パス（自前Square / 自前Stripe / POSLA Connect）
- 分岐ロジック（process-payment.php）
- Stripe Billing（サブスクリプション）
- Stripe Connect（Express Account + Destination Charges）
- Stripe Terminal（JS SDK + ConnectionToken）
...
- charge_square() / charge_stripe() の使い方
