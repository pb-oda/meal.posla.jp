...
| **セルフオーダー** | 顧客がQRコードスキャン → メニュー表示 → 検索/絞り込み → カート → 注文送信 |
...
| **AIシェフ** | KDS画面のAI調理支援パネル。料理品目単位で優先順位分析（urgent/next/waiting）。調理工程の長さ・同時仕上げ・待ち時間を考慮。60秒自動更新 |
...
| **セルフメニュー検索・安全確認・提案（SELF-MENU-4/GUIDANCE）** | セルフオーダー画面でメニュー名/説明検索、人気/早く出る/辛い/アレルギー除外/ベジ/子ども向けのクイック絞り込みを提供。店舗が設定する構造化属性（辛さ、ベジ、子ども向け、早出し、提供目安分）を優先し、未設定時のみ品名/説明キーワード・提供実績へフォールバックする。アレルギー絞り込み中はスタッフ確認ボタンを表示し、`call_alerts` へ「アレルギー確認をお願いします」を送信できる。品切れ更新でカートから外れた品目には `menu_alternatives` の手動代替を最優先し、未設定時は同カテゴリ・おすすめ・人気・早く出る候補から「こちらなら提供できます」を表示。追加注文時は店舗全体のおすすめ/人気に加え、`order_items` の過去60日併売実績から「この料理に合う一品」を表示。注文送信前は品切れ、LO、注文上限、提供不可オプション、アレルギー、長めの待ち時間をカートで確認する |
...
| **メニュー多言語対応（L-4）** | セルフオーダー画面の言語切替は現画面で ja/en/zh-Hans/ko を表示する。`ui_translations` と `menu_translations` は zh-Hant も保存可能だが、`public/customer/menu.html` では 2026-04-29 時点で繁体字表示を出さず zh-Hans にフォールバックする。メニュー名・カテゴリ名・オプション名はmenu_translationsテーブルでキャッシュ管理。UI文言（ボタン・ラベル・検索/絞り込み/提案メッセージ等）はui_translationsテーブルで管理。管理画面「一括翻訳」ボタンでGemini 2.0 Flash APIによる自動翻訳（30件/バッチ）。`_t(key, fallbackJa)` / `_getLocalizedName(obj)` ヘルパー関数。AIウェイター・呼び出しボタン等の外部JSも `window.__posla_lang` / `window.__posla_t` グローバルブリッジで連動。既存のname_enカラムは維持、追加言語はmenu_translationsで管理 |
...
│   │   ├── ai-kitchen.php            # POST AIシェフ（Gemini品目分析）
...
│       ├── menu-alternatives.php      # GET/POST 品切れ時の手動代替提案
│       ├── menu-quality.php           # GET メニュー整備チェック（写真/説明/アレルゲン/タグ/価格）
...
│   │       ├── ai-kitchen.js        # AIシェフ（DOM自己挿入パネル）
...
│   ├── migration-p119d-zh-hant-encoding-fix.sql           # P1-19d: ui_translations zh-Hant checkout系文字化け補正
...
│   ├── migration-self-menu-attributes.sql                 # SELF-MENU-4: menu_templates/store_local_items に検索・絞り込み用属性を追加
│   ├── migration-self-menu-i18n.sql                       # SELF-MENU-4: セルフメニュー検索・絞り込みUI翻訳キーを追加
│   └── migration-self-menu-guidance.sql                   # SELF-MENU-GUIDANCE: 代替提案テーブル + 安全確認UI翻訳キーを追加
├── uploads/
│   └── menu/                          # メニュー画像保存先
├── tools/
...
| `spice_level` | TINYINT | NOT NULL, DEFAULT 0 | 辛さレベル（0=なし、1=控えめ、2=辛い、3=激辛）※ migration-self-menu-attributes |
| `is_vegetarian` | TINYINT(1) | NOT NULL, DEFAULT 0 | ベジ対応フラグ ※ migration-self-menu-attributes |
| `is_kids_friendly` | TINYINT(1) | NOT NULL, DEFAULT 0 | 子ども向けフラグ ※ migration-self-menu-attributes |
| `is_quick_serve` | TINYINT(1) | NOT NULL, DEFAULT 0 | 早出し対象フラグ ※ migration-self-menu-attributes |
| `prep_time_min` | INT | DEFAULT NULL | 店舗が明示する提供目安分 ※ migration-self-menu-attributes |
...
#### 4.3.18b `menu_alternatives` — 品切れ時の手動代替提案

| カラム | 型 | 制約 | 説明 |
|--------|-----|------|------|
| `id` | VARCHAR(36) | PK | UUID |
| `store_id` | VARCHAR(36) | NOT NULL, FK → stores(id) ON DELETE CASCADE | 店舗スコープ |
| `source_item_id` | VARCHAR(36) | NOT NULL | 売り切れ元品目ID（template/local共通） |
| `source_type` | ENUM('template','local') | NOT NULL, DEFAULT 'template' | 売り切れ元の種別 |
| `alternative_item_id` | VARCHAR(36) | NOT NULL | 代替候補品目ID |
| `alternative_source` | ENUM('template','local') | NOT NULL, DEFAULT 'template' | 代替候補の種別 |
| `sort_order` | INT | NOT NULL, DEFAULT 0 | 表示優先順 |
| `created_at` | DATETIME | NOT NULL, DEFAULT CURRENT_TIMESTAMP | |
| `updated_at` | DATETIME | NOT NULL, AUTO UPDATE | `menu-version.php` の変更検知対象 |

**INDEX:** `(store_id, source_item_id)`, `(store_id, alternative_item_id)`, `(store_id, source_item_id, alternative_item_id)` UNIQUE

**用途:** セルフオーダーでカート内品目が途中で品切れになった場合、手動設定された代替候補を自動候補より優先して表示する。売り切れ元品目そのものは顧客メニュー一覧には表示しない。
...
| `overtime_threshold_minutes` | INT | 480 | 残業判定閾値（分） |
| `early_clock_in_minutes` | INT | 15 | 早出打刻許容（分） |
| `auto_clock_out_hours` | INT | 12 | 自動退勤閾値（時間）。cron実行時に超過レコードを自動退勤 |
...
| `target_labor_cost_ratio` | DECIMAL(5,2) | 30.00 | L-3 Phase 4: 公開前チェック/集計サマリーの目標人件費率（%） |
| `created_at` | DATETIME | | |
| `updated_at` | DATETIME | ON UPDATE CURRENT_TIMESTAMP | |
...
| `role_hint` | VARCHAR(50) | NULL | 推奨持ち場。`shift_work_positions.code` を使用。NULL=指定なし |
| `is_active` | TINYINT(1) | DEFAULT 1 | 論理削除 |
| `created_at` | DATETIME | | |
| `updated_at` | DATETIME | ON UPDATE CURRENT_TIMESTAMP | |
...
| `store_id` | VARCHAR(36) | FK → stores(id) ON DELETE CASCADE | |
| `user_id` | VARCHAR(36) | FK → users(id) ON DELETE CASCADE | |
| `shift_assignment_id` | VARCHAR(36) | NULL | 対応するシフト割当ID |
| `clock_in` | DATETIME | NOT NULL | 出勤打刻 |
| `clock_out` | DATETIME | NULL | 退勤打刻（NULL=勤務中） |
| `break_minutes` | INT | DEFAULT 0 | 休憩分数 |
...
| `status` | ENUM | 'working','completed','absent','late' DEFAULT 'working' | 状態 |
| `clock_in_method` | ENUM | 'manual','auto' DEFAULT 'manual' | 出勤打刻方法 |
| `clock_out_method` | ENUM | 'manual','auto','timeout' DEFAULT 'manual' | 退勤打刻方法 |
| `note` | TEXT | NULL | メモ |
| `created_at` | DATETIME | | |
| `updated_at` | DATETIME | ON UPDATE CURRENT_TIMESTAMP | |

出退勤の実績記録。`clock_out_method='timeout'` はcronによる自動退勤。GPS座標はshift_settings.gps_required有効時に記録。
...
| `role_hint` | VARCHAR(20) | NULL | 持ち場コード / NULL |
...
ワークフロー: pending→approved（スタッフ指名→shift_assignments自動作成）/rejected/cancelled。α-1 では本部一括メニュー配信以外の機能は全契約者に標準提供するため、プラン差分では制限しない。
...
#### 4.3.49 `shift_work_positions` — 店舗別持ち場マスタ ※ migration-p1-56
...
| `code` | VARCHAR(20) | NOT NULL, UNIQUE構成要素 | 持ち場コード。英数字、`_`、`-` |
| `label` | VARCHAR(50) | NOT NULL | 画面表示名 |
| `is_active` | TINYINT(1) | DEFAULT 1 | 0=新規選択から非表示 |
| `sort_order` | INT | DEFAULT 100 | 表示順 |
| `created_at` | DATETIME | | |
| `updated_at` | DATETIME | ON UPDATE CURRENT_TIMESTAMP | |

初期値として各店舗に `hall`（ホール）と `kitchen`（キッチン）をINSERTする。`api/store/shift/positions.php` は未登録店舗に対して GET 時も初期値を補完する。`shift_templates.role_hint` / `shift_assignments.role_type` / `shift_help_requests.role_hint` はこの `code` を参照するが、履歴保持のためDB外部キーは張っていない。

#### 4.3.50 `shift_swap_requests` — 同店舗シフト交代/欠勤申請 ※ migration-p1-56
...
| `shift_assignment_id` | VARCHAR(36) | NOT NULL, INDEX | 対象シフト |
| `request_type` | ENUM('swap','absence') | DEFAULT 'swap' | 交代依頼 / 欠勤連絡 |
| `requester_user_id` | VARCHAR(36) | NOT NULL | 申請対象スタッフ。staff申請時は本人のシフトのみ |
| `candidate_user_id` | VARCHAR(36) | NULL | 申請時に希望した交代候補 |
| `replacement_user_id` | VARCHAR(36) | NULL | 承認時に確定した交代スタッフ |
| `status` | ENUM('pending','approved','rejected','cancelled') | DEFAULT 'pending' | 申請状態 |
| `reason` | TEXT | NULL | 申請理由 |
| `response_note` | TEXT | NULL | 店長の対応メモ |
| `responded_by` | VARCHAR(36) | NULL | 承認/却下/キャンセルしたユーザー |
| `created_at` | DATETIME | | |
| `updated_at` | DATETIME | ON UPDATE CURRENT_TIMESTAMP | |
...
| エンドポイント | メソッド | 認証 | 説明 |
|---------------|---------|------|------|
| `unified-view.php` | GET | owner | L-3 Phase 3: 全店舗シフト横断サマリー。?period=weekly\|monthly&date=YYYY-MM-DD。店舗別の出勤人数・人件費・充足率・ヘルプ要請件数を一括返却 |
...
| `menu-alternatives.php` | GET/POST | manager+ | 品切れ時の手動代替提案。GETで現在の表示メニューと設定済み代替を返し、POSTで売り切れ元1品目分の代替候補を全置換する |
| `menu-quality.php` | GET | manager+ | メニュー整備チェック。写真なし、説明なし、アレルゲン未設定、タグ未設定、価格未設定を集計し、品目別に返す |
...
- シフト概況（L-3 Phase 3）: 個別店舗サマリー + 全店舗統合ビュー（出勤人数・人件費・充足率・ヘルプ要請件数。人件費比較バーチャート）
...
**品切れ時の代替提案（SELF-MENU-GUIDANCE）:**
- `menu-alternative-editor.js` — IIFE モジュール
- dashboard.html の「店舗メニュー」セクション内に `<details>` で配置
- 売り切れ元1品目に対して、代替候補を最大3件まで選択
- 保存時は `api/store/menu-alternatives.php` にPOSTし、その売り切れ元の設定を全置換
- 顧客画面では `menu_alternatives` の候補を最優先し、未設定時のみ自動候補へフォールバック

**メニュー整備チェック（SELF-MENU-GUIDANCE）:**
- `menu-quality-check.js` — IIFE モジュール
- dashboard.html の「店舗メニュー」セクション内に `<details>` で配置
- `api/store/menu-quality.php` から、写真なし/説明なし/アレルゲン未設定/タグ未設定/価格未設定を取得
- 画面上部に件数サマリー、下部に品目別の不足項目を表示
- CSV取込後、季節メニュー追加後、公開前チェックで使う想定。全てをゼロにする運用ではなく、セルフメニューでお客様が迷う原因を見つけるための確認画面
...
QRスキャン → table-session API（自動着席）→ ウェルカムモーダル → メニュー表示
  → 検索/絞り込み/カテゴリ選択（「おすすめ」タブ含む）→ 品目タップ → オプションモーダル（あれば）
...
- **検索・絞り込み (SELF-MENU-4):** `public/customer/menu.html` 内で取得済みメニューに対してクライアント側フィルタリング。対象はメニュー名/説明/カテゴリ名、人気、早く出る、辛い、アレルギー除外、ベジ、子ども向け。AI判定は使わず、店舗が登録した構造化属性（`spice_level`, `is_vegetarian`, `is_kids_friendly`, `is_quick_serve`, `prep_time_min`）と、既存の人気ランキング・おすすめ設定・アレルゲン設定・品名/説明キーワード・品目別提供実績から判定する。構造化属性がある場合はそれを優先する
- **店舗全体の待ち時間目安 (SELF-MENU-4):** `table-session.php` の `avgWaitMinutes` がある場合、検索欄下に「現在の目安: 約X分」を表示する。注文後の進捗は既存の注文トラッカーで確認できるため、待ち時間表示は期待値調整に限定する
- **品目別目安提供時間 (SELF-MENU-4):** `api/lib/menu-resolver.php` が品目の `prepTimeMin` を返す。店舗が提供目安分を設定している場合はその値を品目カードに表示する。未設定の場合のみ、`api/customer/menu.php` の `prepTimes` を使い、過去30日の `order_items` で `status IN ('ready','served')`、`ready_at` または `served_at` がある実績を品目別に平均し、サンプル2件以上のときだけ `avgMinutes` として表示する。実績不足時は表示しない
- **アレルギー確認導線 (SELF-MENU-GUIDANCE):** アレルギー絞り込みを選ぶと、除外チップと同じパネル内に「スタッフに確認する」ボタンを表示する。`public/customer/js/call-staff.js` は `CallStaff.request(reason)` を公開し、押下時は `api/customer/call-staff.php` に「アレルギー確認をお願いします」を送信する。これはセルフ除外を補助する導線であり、アレルギー最終判断を顧客だけに委ねないためのもの
- **オプション明確化 (SELF-MENU-GUIDANCE):** `menu-resolver.php` は `store_option_overrides.is_available = 0` の選択肢も `isAvailable:false` で返す。顧客画面では必須/任意、必要数、選択上限、選択済み数、価格差、売り切れ選択肢を表示し、提供不可オプションはdisabledにする。`order-validator.php` でも `OPTION_UNAVAILABLE` でサーバー側再検証する
- **代替提案 (SELF-MENU-GUIDANCE):** 品切れ即時反映でカート内品目が削除された場合、`menu_alternatives` の手動代替候補を最優先し、未設定時のみ同カテゴリ・今日のおすすめ・人気ランキング・早く出る候補を優先して「こちらなら提供できます」を表示する。売り切れ品そのものはメニュー一覧には引き続き表示しない
- **追加注文おすすめ (SELF-MENU-4):** 同一セッションに注文履歴がある場合、前回の注文セクションとは別に店舗全体のおすすめ/人気候補を「もう一品いかがですか」として表示する。卓別の嗜好推定は使わず、説明しやすい全体レコメンドに限定する
- **併売レコメンド (SELF-MENU-GUIDANCE):** カート追加後に、`api/customer/menu.php` の `pairRecommendations` を使って「この料理に合う一品」を表示する。集計元は過去60日の `order_items` 同一注文内ペアで、support 2件以上、品目あたり最大3件。AI生成ではなく実績データのみを使う
- **注文送信前チェック (SELF-MENU-GUIDANCE):** カートモーダル先頭に「送信前の確認」を表示する。LO終了、品切れ/削除済み品目、提供不可オプション、注文上限品数、注文上限金額は送信ブロック。アレルギー情報、アレルギー除外フィルタ、提供目安20分以上の品目は注意表示のみで送信は止めない
...
- バージョン変更検知時のみフルメニュー再取得（recommendations/popularity/prepTimes/alternativeMap/pairRecommendationsも更新）
- `menu-version.php` の対象は `store_menu_overrides`, `store_local_items`, `menu_alternatives` の `updated_at`
- カート内の品切れ品目を自動検出・削除し、非ブロッキングのトースト通知と「こちらなら提供できます」候補を表示
...
- ステーション状況帯の下にKDS端末状態を常時表示。最終ポーリング時刻、通信遅延、stale/live、音声認識の最終成功時刻、通信異常を小さく表示し、未更新/20秒超/stale/エラーは赤、10秒超は黄にする
...
- ヘッダーの `着手時間` で、予約/テイクアウトの事前表示をテイクアウト/予約/カテゴリ別に端末設定できる。設定は `localStorage: mt_kds_prep_lead_v1` に保存し、カテゴリ別設定は注文APIの品目 `category_id` / `category_name` を使って適用する
...
- **KdsRenderer** — カンバンレンダラー。ステータス遷移ボタン付き注文カード。テイクアウト注文は「テイクアウト」バッジ（オレンジ拡大）+受取時間バッジ+薄オレンジ背景グラデーションで視認性強化。経過時間、予約時刻、テイクアウト受取時刻からnormal/prep/warning/dangerを判定し、`getOperationalSummary()`でステーション状況帯用の集計を返す。`setPrepLeadConfig()` で端末側の着手時間設定を受け取り、カテゴリ別に予約/テイクアウトの事前表示タイミングを変える
- **SoldOutRenderer** — 品切れトグル画面（30秒ポーリング）+ 音声コマンド（品切れトグル・一覧表示）
- **VoiceCommander** — 音声コマンダー（`voice-commander.js`）
- **AiKitchen** — AIシェフ（`ai-kitchen.js`）。Gemini APIで料理品目単位の調理優先順位を分析。ボトムパネルUI、60秒自動更新。`GET /api/kds/orders.php` から注文取得→品目展開→`POST /api/kds/ai-kitchen.php` に送信
...
音声でKDSのステータス更新を行うハンズフリーモジュール。Web Speech APIを入口にし、厨房で頻出する操作はGeminiを使わないファストパスで処理する。Geminiは端末ごとの「AIシェフON」時だけ、ファストパスで処理できない曖昧な発話の補助解析に使う。
...
| **AIシェフON/OFF** | ヘッダーの `AIシェフON / AIシェフOFF` ボタンで切替。保存先は `localStorage: mt_kds_ai_chef_enabled`。互換のため `mt_kds_voice_ai_fallback` にも同期するが、KDS起動時は保存済みONを復元せず必ずOFFから始める。OFF時は未対応の言い回しをGeminiへ送らず停止、ON時だけAIシェフパネルとGeminiフォールバックを使う。AI設定不備は店舗側設定ではなくPOSLA運営側障害として裏側で監視アラートに出し、利用者画面では赤い障害表示にせず「AIシェフは現在使用できません」と中立表示する |
| **音声診断** | ヘッダーに `音声診断` ボタンを表示。音声でも「音声診断」「マイク診断」「音声状態」「マイク状態」で開く。端末内 `localStorage: mt_kds_voice_diag_v1` に成功/失敗、final回数、AIシェフ使用、自動リフレッシュ、no-speech/network/audio-capture回数を保存。診断画面を開いている間だけWeb Audio APIでマイク入力レベルを表示し、閉じるとMediaStreamを停止する |
...
| **Gemini解析** | AIシェフON時のみ、ファストパスで処理できない発話を `POST ../../api/store/ai-generate.php` に送信。注文一覧とメニュー一覧をコンテキストとして渡し、JSONのみを返させる |
...
GET /customer/menu?store_id=X (&plan_id=P | &course_id=C)
  ...
  │ → recommendations（今日のおすすめ）+ popularity（人気ランキング）+ prepTimes（品目別目安提供時間）も返却
  ▼
...
SOURCE migration-p119d-zh-hant-encoding-fix.sql;      -- ui_translations zh-Hant checkout系文字化け補正
...
-- SELF-MENU-4 セルフメニュー検索・絞り込み
SOURCE migration-self-menu-attributes.sql;            -- menu_templates/store_local_items に検索・絞り込み属性を追加
SOURCE migration-self-menu-i18n.sql;                  -- セルフメニュー検索・絞り込みUI翻訳キーを追加
SOURCE migration-self-menu-guidance.sql;              -- menu_alternatives + 安全確認/代替/併売/最終確認UI翻訳キーを追加
...
- L-3（シフト管理）は全契約者に標準提供。`check_plan_feature()` は `hq_menu_broadcast` 以外を常に true にするため、シフト系 cron はプラン差分を理由に止まらない
...
| `mt_kds_ai_chef_enabled` | KDS | AIシェフON状態。AIシェフパネルと未対応音声のGemini補助解析をまとめて切替。実行中の同期用で、KDS起動時は復元せずOFFに初期化 |
| `mt_kds_voice_ai_fallback` | KDS | 旧AI補助状態との互換キー。現行UIでは `mt_kds_ai_chef_enabled` と同期し、KDS起動時は削除 |
| `mt_kds_voice_diag_v1` | KDS | 端末内の音声診断ログ。成功/失敗、final回数、自動リフレッシュ回数、エラー回数など |
| `mt_kds_prep_lead_v1` | KDS | 予約/テイクアウトの着手時間設定。テイクアウト/予約/カテゴリ別の分数を端末ごとに保存 |
