...
- ヘッダーの `着手時間` で、予約/テイクアウトの事前表示を全体/予約/テイクアウト/カテゴリ別に端末設定できる。設定は `localStorage: mt_kds_prep_lead_v1` に保存し、カテゴリ別設定は注文APIの品目 `category_id` / `category_name` を使って適用する
...
| `mt_kds_prep_lead_v1` | KDS | 予約/テイクアウトの着手時間設定。全体/予約/テイクアウト/カテゴリ別の分数を端末ごとに保存 |
