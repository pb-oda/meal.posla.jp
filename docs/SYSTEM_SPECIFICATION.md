...
| **AI音声コマンド** | KDS画面でハンズフリー操作。Web Speech API + Gemini APIで音声→ステータス更新。品切れ管理画面でも音声操作（品切れトグル・一覧表示）。ノイズフィルタリング・3分自動リフレッシュ・確認ポップアップ・ファストパス対応 |
...
| **決済ゲートウェイ連携** | Stripe対面決済端末（Stripe Reader S700）連携。Stripe Terminal JS SDKでWi-Fi経由端末自動検出。Stripe Connect（POSLA決済）または自前Stripeアカウントに対応。テナント単位APIキー設定。決済中オーバーレイ+結果トースト。graceful degradation（キー未設定時はローカル記録のみ）。※Squareはサーバーサイド記録のみ（物理端末非連携）、将来的にSquare Terminal SDK対応を検討 |
| **Googleレビュー連携** | 満足度4点以上の顧客にGoogleマップレビューリンクを表示。store_settings.google_place_id設定。6秒トースト+多言語対応 |
| **事前注文・テイクアウト予約（L-1）** | 顧客がURLから事前にテイクアウト注文。受取時間15分刻みスロット管理。オンライン決済（Stripe Checkout/Square Payment Links）対応。店舗ダッシュボードでテイクアウト注文管理タブ。ステータス追跡（受付→調理中→準備完了→受取済）。レート制限・冪等キー・品切れチェック。KDSカード視認性強化（「テイクアウト」バッジ+受取時間+背景グラデーション）。セルフオーダーmenu.htmlからテイクアウト導線リンク（有効店舗のみ表示）。KDS品目ready時ハンディ通知対応（table_id=NULL→LEFT JOIN+ダミーテーブル'TAKEOUT'で通知作成） |
...
| **セッションアイドルタイムアウト** | 30分操作なしで自動ログアウト。マウス/キー/タッチ/スクロールでアクティビティ検出。session-monitor.jsで実装 |
...
| **セルフレジ（L-8）** | セルフオーダー画面（menu.html）から直接オンライン決済。store_settings.self_checkout_enabledで店舗別ON/OFF。「お会計」ボタン→未払い注文一覧+税率別内訳モーダル→「カードで支払う」→Stripe Checkout（Connect/直接自動分岐）→決済完了→注文paid更新+セッション終了+在庫減算（トランザクション+冪等性）。完了画面で「領収書を表示」→30分限定お会計明細HTML表示（スクリーンショット/AirPrint対応）。紙の領収書はスタッフ発行（L-5）。5言語翻訳対応。QR決済なし・クレジットカードのみ |
...
| **サブスクリプション課金（L-11）** | Stripe Checkout+Customer Portal+Webhook。3段階プラン（standard ¥10,000/pro ¥20,000/enterprise ¥50,000）。posla_settingsでStripe Billingキー一元管理。subscription_eventsテーブルでイベント記録。オーナーダッシュボード「契約・プラン」タブ |
...
| **GPS出退勤・スタッフツール制御（L-3b）** | 店舗単位のGPS出退勤制御（shift_settings.gps_required/store_lat/store_lng/gps_radius_meters）。Haversine距離計算で打刻時に店舗圏内判定。スタッフ表示ツール制御（shift_settings.staff_visible_tools: CSV形式でhandy/kds/registerの表示制御）。GPS設定画面に「📍 現在地から店舗位置を設定」ボタン（Geolocation API、全幅・primary表示で目立つUI） |
| **ユーザー単位ツール表示制御（L-3b2）** | user_stores.visible_tools（CSV: handy,kds,register、NULL=店舗設定に従う）で個別設定。優先順位: ユーザー個別設定 > 店舗デフォルト設定 > 全表示。ユーザー編集モーダルに「表示ツール」チェックボックス（「店舗設定に従う」トグル付き）。owner/users.phpはvisible_tools_by_storeマップ、staff-management.phpは直接visible_toolsで管理。me.phpがstores配列にuserVisibleToolsを返却 |
...
| **staff** | 1 | 割当店舗の操作のみ、ハンディ注文、KDS操作、テーブル管理 |
...
│   ├── js/
│   │   ├── session-monitor.js         # セッションアイドルタイムアウト監視（30分）
...
| `payment_gateway` | ENUM('none','square','stripe') | NOT NULL, DEFAULT 'none' | 使用する決済ゲートウェイ（C-3） |
| `square_access_token` | VARCHAR(200) | DEFAULT NULL | Square APIトークン（C-3） |
| `stripe_secret_key` | VARCHAR(200) | DEFAULT NULL | Stripe Secret Key（C-3、店舗自前） |
| `square_location_id` | VARCHAR(100) | DEFAULT NULL | Square Location ID（テイクアウト決済用） |
...
POSLA運営が一元管理する共通設定。初期キー: `gemini_api_key`, `google_places_api_key`。L-11で追加: `stripe_secret_key`, `stripe_publishable_key`, `stripe_webhook_secret`, `stripe_price_standard`, `stripe_price_pro`, `stripe_price_enterprise`。L-13で追加: `stripe_connect_*` 関連。L-15で追加: `smaregi_client_id`, `smaregi_client_secret`。POSLA管理画面（`/public/posla-admin/`）から設定。
...
- **用途:** `session-monitor.js` からのアイドルタイムアウト確認。30分無操作時のセッション有効性検証
...
```
- **処理:**
  1. `stores` + `tenants` JOIN で `ai_api_key` 取得（サーバーサイド、フロントに非露出）
...
```
- **エラー:** `AI_NOT_CONFIGURED`(503) — テナントに `ai_api_key` 未設定、`GEMINI_ERROR`(502) — Gemini API通信エラー
...
- **POST:** テイクアウト注文作成
  ...
  - オンライン決済時は Stripe/Square のチェックアウトURLを返す
- **レート制限:** 10 req/h per key

#### GET `/api/customer/takeout-payment.php`
- **認証:** 不要（L-1 テイクアウト）
- **パラメータ:** `?order_id=xxx&session_id=xxx`（Stripe）または `?order_id=xxx&transaction_id=xxx`（Square）
- **処理:** オンライン決済の確認。Stripe/Square API に問い合わせ、支払い完了時に `takeout_orders.status` を `pending_payment` → `pending` に更新し、`payments` テーブルに記録
- **レスポンス:**
```json
...
- **処理フロー:**
  1. `tenants.ai_api_key` からGemini APIキーを取得
...
| `tenants.php` | PATCH | owner | テナント名・APIキー（ai_api_key, google_places_api_key）更新。キー値を空文字で送信すると削除 |
...
| `process-payment.php` | POST | staff+ | POSレジ会計処理。cash/card/qr対応。外部決済ゲートウェイ連携（Square/Stripe、C-3）。gateway情報DB記録。合流/分割/個別会計対応 |
...
#### dashboard.html（店舗運営）
- ヘッダー: テナント名、店舗セレクター、ショートカットリンク（ハンディ/KDS/レジ）、「注文画面」リンク（スタッフには非表示）
...
- ツール表示制御（L-3b/L-3b2）: staffロールの場合、`userVisibleTools`（個別設定）→ `staffVisibleTools`（店舗設定）→ 全表示の優先順位でハンディ/KDS/レジのヘッダーショートカットとスタッフホームボタンを非表示制御
...
- **KdsRenderer** — カンバンレンダラー。ステータス遷移ボタン付き注文カード。テイクアウト注文は「テイクアウト」バッジ（オレンジ拡大）+受取時間バッジ+薄オレンジ背景グラデーションで視認性強化
...
音声でKDSのステータス更新を行うハンズフリーモジュール。Web Speech API + Gemini API連携。
...
| **コマンド検出** | テーブル番号パターン(`T01`等) or 動作語（調理/完成/提供/済み等）を含む発話のみ処理 |
| **interim確定** | コマンドパターン検出後1.5秒タイマーで確定（Chrome が final result を返さないケース対策） |
| **自動リスタート** | `onend` で300ms後に再開（Chrome無音タイムアウト対策） |
| **3分自動リフレッシュ** | Chrome長時間セッションで認識精度が劣化する問題対策。3分ごとに `SpeechRecognition` インスタンスを再作成 |
| **ノイズフィルタリング** | キッチン雑音対策。最小文字数(2)・最大文字数(60)・最低信頼度(0.4)でフィルタ。短い/長い/低信頼度の結果を除去 |
| **Gemini解析** | 注文一覧JSONをコンテキストとして送信。temperature=0。テーブル特定はGeminiに委任 |
| **確信度** | Geminiが `confidence: "low"` を返した場合、confirm()ダイアログで確認 |
| **品切れファストパス** | 「品切れ一覧」「品切れ確認」「今の品切れ」「品切れリスト」はGeminiを経由せず即座にローカル実行 |
...
| **APIキー** | テナント単位。`settings.php?include_ai_key=1` で取得。店舗変更時に再取得 |
...
**session-monitor.js:**
- セッションアイドルタイムアウト（30分）
- マウス移動・キー入力・タッチ・スクロールでアクティビティ検出
- 401レスポンス中に `SESSION_TIMEOUT` を検出した場合、ログイン画面にリダイレクト
...
| **アイドルタイムアウト** | 30分無操作で自動ログアウト（`session-monitor.js` + `auth/check.php`） |
