...
| **音声診断** | ヘッダーに `音声診断` ボタンを表示。音声でも「音声診断」「マイク診断」「音声状態」「マイク状態」で開く。端末内 `localStorage: mt_kds_voice_diag_v1` に成功/失敗、final回数、AI補助使用、自動リフレッシュ、no-speech/network/audio-capture回数を保存 |
...
- 提供待ち品目を持つ注文を先頭、次に赤/黄優先度、最後に経過時間が長い順で表示
