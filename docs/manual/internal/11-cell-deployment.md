...
OP監視用DB read-onlyユーザー:

| 変数 | 用途 |
|---|---|
| `POSLA_OPS_DB_READONLY_USER` | OP / runner がDBを読むためのSELECT専用ユーザー。省略時 `posla_ops_ro` |
| `POSLA_OPS_DB_READONLY_PASSWORD` | SELECT専用ユーザーのパスワード。`provision-ready-cells.php` は自動生成する |
| `POSLA_OPS_DB_READONLY_HOST` | MySQL user のHost部。擬似本番は `%`、本番はOP/runnerの接続元に合わせて制限する |
...
テナント画面の障害報告窓口から、障害報告を codex-ops-platform の障害報告へ送れます。
`monitor-health.php` で検知した監視アラートも、同じ OP へ送れます。アラートを OP へ送ると、OP 側で分類後に自動 Investigation を作成し、runner read-only 調査へ進みます。

POSLA側:

- API: `/api/support/op-case.php`
- 監視: `/api/cron/monitor-health.php`
- POSLA管理画面: `API設定 > OP障害報告 / 監視アラート連携`
...
- 障害報告 Endpoint: `https://<op-domain>/api/cases.php`
- 監視アラート Endpoint: `https://<op-domain>/api/alerts.php`
