...
監視は OP が POSLA の ping / snapshot を read-only で読む方向を正とします。POSLA 管理画面では OP 画面URLと手動障害報告の送信先だけを扱い、監視対象・Google Chat通知・Alert再判定は OP 側で管理します。

POSLA側:

- API: `/api/support/op-case.php`
- POSLA管理画面: `設定センター > OP起動・障害報告`
...
- 障害報告 header: `X-OPS-CASE-TOKEN`
- 障害報告 Token: OP 側 `Settings > POSLA接続` で発行し、POSLA管理画面にも同じ値を保存
