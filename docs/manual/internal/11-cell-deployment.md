...
### 11.11.0 Support Case bridge

テナント画面のサポートFABから、問い合わせを codex-ops-platform の Support Cases へ送れます。

POSLA側:

- API: `/api/support/op-case.php`
- POSLA管理画面: `API設定 > OP問い合わせ連携`
- 設定キー: `codex_ops_case_endpoint`, `codex_ops_case_token`
- env override: `POSLA_OP_CASE_ENDPOINT`, `POSLA_OP_CASE_TOKEN`

OP側:

- Endpoint: `https://<op-domain>/api/cases.php`
- env: `OPS_CASE_INGEST_TOKEN`
- header: `X-OPS-CASE-TOKEN`
...
- store_id / store_name
- errorNo / 問い合わせ本文 / 発生時刻
- 画面名 / 画面URL / ログインユーザー / role / user_agent

`codex_ops_case_token` は秘密値として扱い、GETでは実値を返しません。endpoint は非秘密値として管理画面に表示します。
