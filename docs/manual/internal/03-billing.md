...
::: tip ログ確認の canonical
課金系の調査は、まず `docker compose logs php` と `POSLA_PHP_ERROR_LOG` を見ます。共有サーバ向けの SSH grep 例は歴史的参考情報です。
:::

::: tip 2026-04-09 α-1 構成（確定）
2026-04-09 より、**単一プラン + アドオン**構成に切り替わりました。本章は α-1 構成で書かれています。機能判定（`check_plan_feature`）は α-1 ベースで一本化済（`hq_menu_broadcast` のみ判定、それ以外は常に true）。現在の POSLA 管理画面は **新規テナントを `POSLA標準` 固定で作成** し、旧 `standard / pro / enterprise` は legacy 表示互換としてのみ扱います。
:::
...
5. POSLA管理画面 `/posla-admin/dashboard.html` の「API設定」タブに登録
...
2. URL に本番ドメインの webhook を入力：`https://<production-domain>/api/subscription/webhook.php`
...
POSLA 管理画面の新規作成 UI からは、旧 `standard/pro/enterprise` ドロップダウンは撤去済みです。既存 legacy データだけが `plan_compat` として互換表示されます。
