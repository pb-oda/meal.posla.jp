...
社内向けビルドは `VP_MODE=internal vitepress build` で生成し、`public/docs-internal/` に出力され、配信 URL は `/docs-internal/` です。顧客向けビルドは `VP_MODE=tenant vitepress build` で生成し、`tenant/` と現場手順の `operations/` を公開対象にし、`internal/**` と管理用ステータスファイルを除外します。
...
- CORS許可は `POSLA_ALLOWED_ORIGINS` env 経由で設定し、本番では `<production-domain>` を canonical として明示します。詳細は `internal/production-api-checklist.md §9-0` を参照。
...
| 分類 | テーブル |
|---|---|
| テナント・店舗 | `tenants`, `stores`, `store_settings`, `posla_settings`。`store_settings` は営業日区切り、レジ、プリンタ、テイクアウト枠、ブランド色、`brand_logo_url`, `brand_display_name` を含む |
...
| シフト | `shift_settings`, `shift_templates`, `shift_availabilities`, `shift_assignments`, `shift_help_requests`, `shift_help_assignments`, `attendance_logs`, `shift_work_positions`, `shift_swap_requests`, `shift_request_messages`, `shift_request_notifications`, `shift_open_shifts`, `shift_open_shift_applications`, `shift_skill_tags`, `shift_staff_skill_tags`, `shift_position_required_skills`, `shift_task_templates`, `shift_task_assignments`, `shift_availability_announcements`, `shift_availability_announcement_reads` |
| 予約 | `reservation_settings`, `reservations`, `reservation_customers`, `reservation_holds`, `reservation_courses`, `reservation_notifications_log`, `reservation_change_logs`, `reservation_waitlist_candidates` |
| テイクアウト | `orders` の `order_type='takeout'` と `pickup_at`, `takeout_pack_checklist`, `takeout_ready_notification_status`, `takeout_ops_status`, `takeout_arrived_at` などの列、`takeout_order_line_links`, `takeout_order_line_link_tokens` |
...
| 管理画面 | `public/admin/dashboard.html` | `api/store/*`, `api/store/admin-ops.php` | 多数 | staffはシフト中心。manager は運営管理でコックピット、横断検索、売上ドリル、初期設定、端末稼働、カート離脱を確認 |
| オーナー画面 | `public/admin/owner-dashboard.html` | `api/owner/*` | `tenants`, `stores`, `users`, `menu_templates` | ownerでも同一tenant検証必須 |
| メニュー | dashboard各メニューJS | `api/owner/menu-templates.php`, `api/store/menu-overrides.php`, `api/store/local-items.php`, `api/store/menu-overrides-csv.php`, `api/store/local-items-csv.php` | メニュー系テーブル | 本部マスタ、店舗上書き、店舗限定を混同しない。店舗メニューCSVは `template_id,name,category_name,base_price,price,is_hidden,is_sold_out,image_url` |
...
| テイクアウト | `public/customer/takeout.html` / 管理画面「テイクアウト」 | `api/customer/takeout-orders.php`, `api/customer/takeout-payment.php`, `api/customer/takeout-line-token.php`, `api/store/takeout-management.php` | `orders`, `payments`, `store_settings`, `takeout_order_line_links`, `takeout_order_line_link_tokens` | オンライン決済、受取枠、品数上限、ピーク上限、梱包、到着連絡、LINE準備完了通知、返金ステータスを確認。テイクアウトSMSは未実装 |
...
| シフト | dashboard / スタッフ画面 | `api/store/shift/*` | シフト系テーブル、`attendance_logs` | deviceを人件費集計に入れない。希望提出依頼、申請メッセージ、未読通知、候補本人承諾、空きシフト、担当作業、スキル、当日・ヘルプを含む |
| レポート | dashboard、owner | `api/store/sales-report.php`, `api/store/admin-ops.php?action=sales_drilldown`, `api/owner/cross-store-report.php` | `orders`, `payments`, `audit_log`, `cart_events` | 営業日区切りに注意。売上ドリルは日別→商品→注文→会計履歴、カート離脱は `cart_events` と注文内キャンセルを読む |
...
| 予約 | customer、dashboard | `api/customer/reservation-create.php`, `api/customer/reservation-update.php`, `api/customer/reservation-cancel.php`, `api/store/reservations.php`, `api/store/reservation-settings.php`, `api/store/reservation-notify.php` | `reservations`, `reservation_settings`, `reservation_customers`, `reservation_holds`, `reservation_change_logs`, `reservation_waitlist_candidates`, `reservation_notifications_log` | 二重予約、予約金、変更履歴、リマインド再送、SMS Webhook、キャンセル待ち仮押さえの扱い |
...
- 管理画面と、デバイスアカウントで使うKDS・ハンディはPWA化対象。
- 本番環境では `public/admin/`, `public/kds/`, `public/handy/` に `manifest.webmanifest`, `sw.js`, `pwa-register.js` を配信する。
...
管理ダッシュボードでは、少なくとも次のサブタブを開いて、空データ時にも画面が崩れないことを確認します。

- 運営管理: 運営コックピット、全体検索、売上ドリル、初期設定、端末稼働、カート離脱
- メニュー管理: カテゴリ、オプション、店舗メニュー、限定メニュー、CSV一括更新パネル
- 予約管理: 予約台帳、顧客台帳、コース、予約設定、予約レポート
- シフト管理: 週間カレンダー、スタッフ画面、希望提出、勤怠、当日・ヘルプ
- テイクアウト: 注文一覧、梱包チェック、ステータス操作、通知状態
