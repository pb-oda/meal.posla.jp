...
| POSLA -> OP | `POST /api/cases.php` | 障害報告窓口からの報告送信 | `X-OPS-CASE-TOKEN` |
...
| `POSLA_OP_CASE_TOKEN` | OP側 `OPS_CASE_INGEST_TOKEN` と同じ値 |
...
tenant画面の障害報告窓口から報告されると、OPの `cases.php` が受け取ります。
...
| AI未設定の状態でAI機能をテストした `E9004` | 設定未投入が原因なら解決済み |
