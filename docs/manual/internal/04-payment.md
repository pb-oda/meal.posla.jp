...
# 4. 決済アーキテクチャ（POSLA決済 / 外部決済記録）

POSLAの決済は、2026-04-28 以降、通常レジとオンライン決済を明確に分離します。

- 通常レジ: 店舗既存の現金 / カード端末 / QR決済を使い、POSLAは会計記録のみ残す。
- セルフ会計 / テイクアウト: Stripe Connect を使う POSLA決済として扱う。
- Stripe Terminal による店頭カード決済は新規利用停止。既存コード・カラムは履歴互換として残す。
...
| 使える機能 | セルフ会計、テイクアウト（移行互換） | セルフ会計、テイクアウト |
...
```json
{
  ...
  "terminal_pattern": null,
  "tenant_stripe_configured": false
}
```

- `terminal_pattern`: 現行方針では常に `null`
- `tenant_stripe_configured`: `stripe_secret_key` が設定されているか

`terminal_pattern` は後方互換フィールドとして残しますが、現行のレジアプリ（`cashier-app.js`）は Stripe Terminal SDK を初期化しません。`/api/connect/status.php` は通常レジ向けには常に `terminal_pattern: null` を返します。
...
主な呼び出し元:
- `api/customer/checkout-session.php`（セルフ会計）
- `api/customer/takeout-orders.php` / `api/customer/takeout-payment.php`（テイクアウト）
- `api/store/refund-payment.php`（POSLA決済の返金時）

`api/store/terminal-intent.php` は後方互換のためファイルを残していますが、新規 PaymentIntent 作成は `PAYMENT_NOT_AVAILABLE` で停止します。
...
## 4.3 Stripe Terminal（カードリーダー）廃止方針

通常レジでは Stripe Terminal を使いません。店舗のカード端末・QRアプリで支払いを完了し、POSLAには `payment_method='card'` / `payment_method='qr'` の記録だけを残します。

### 4.3.1 PaymentIntent 生成（terminal-intent.php）

`POST /api/store/terminal-intent.php` は、2026-04-28 以降、新規作成を停止しています。

- 認証と `store_id` 境界は従来通り確認する
- その後 `PAYMENT_NOT_AVAILABLE` を返す
- 古いクライアントで「Stripe決済は完了したがPOSLAには記録できない」事故を防ぐための停止点
...
```json
{
  ...
  "error": {
    ...
    "message": "通常レジのカード決済は店舗の外部決済端末で完了し、POSLAには記録のみ残してください。"
  }
}
```
...
通常レジのカード/QRはPOSLAが決済を実行していないため、Stripe Refund の対象ではありません。店舗の外部決済端末・QRアプリ側で返金し、POSLAでは会計取消で売上記録を調整します。

セルフ会計・テイクアウトのPOSLA決済は Stripe Connect 経由のため、`refund-payment.php` の対象になります。
...
::: warning 解除後の挙動
解除後、セルフ会計・テイクアウトのPOSLA決済は利用できません。通常レジのカード/QRは店舗既存決済の記録専用なので影響しません。
:::
...
会計処理の中核 API。通常レジの支払方法（cash / card / qr）を扱います。card / qr は **外部決済完了後の記録** であり、この API から Stripe 決済は実行しません。2026-04-29 以降は `payment_method_detail` でカード種別、QRブランド、電子マネー種別も記録します。
...
```json
{
  ...
  "payment_method": "qr",
  "payment_method_detail": "qr_paypay",
  "order_ids": ["o-xxx-1", "o-xxx-2"],
  "payment_entry_mode": "manual",
  "selected_items": null,
  "total_override": null
}
```
...
| `payment_method_detail` | — | `card_credit` / `card_debit` / `qr_paypay` / `emoney_transport_ic` 等。未指定時は card=`card_other`、qr=`qr_other` |
...
| `payment_entry_mode` | — | `manual` のみ有効。`gateway` は通常レジでは `PAYMENT_NOT_AVAILABLE` |
...
4. payment_method 別処理:
   ...
   - `card`/`qr`: 店舗の外部決済が完了した前提で、POSLAには記録のみ保存。`payment_method_detail` にブランド / 種別を保存
5. `orders.status = 'paid'`, `paid_at = NOW()`
6. `payments` テーブルに 1 レコード INSERT。P1-46 適用済み環境では `payments.payment_method_detail` / `orders.payment_method_detail` も保存
...
| `PAYMENT_NOT_AVAILABLE` | 400 | 通常レジで `payment_entry_mode='gateway'` または Terminal 決済を要求した |

### 4.8.6 Stripe Terminal の retrieve 検証フロー（履歴）

この節は 2026-04-28 以前の履歴です。現行の `process-payment.php` は `payment_method='terminal'` を受け付けず、`terminal_payment_id` も使いません。
...
│   ├─ 外部決済端末・QRアプリ側を確認
│   └─ POSLAは決済実行せず記録のみ
├─ セルフ会計・テイクアウトのPOSLA決済が失敗
│   ├─ Connect未完了 → 4.11.2
│   └─ Stripe Checkout / webhook 失敗 → 4.11.5
├─ 返金エラー
...
### 4.11.2 POSLA決済が利用できない

**原因**: Connect 未完了、`charges_enabled=0`、または POSLA決済設定がOFF。
...
**対処**: テナント owner に決済設定タブで Stripe Connect オンボーディング完了を依頼。通常レジのカード/QRはこの設定に依存しない。

### 4.11.3 通常レジで `PAYMENT_NOT_AVAILABLE`

**原因**: 古いクライアントまたは手動API呼び出しで、通常レジから gateway / Terminal 決済を実行しようとしている。

**対処**: ブラウザを更新し、カード/QRは外部決済完了後の記録として処理する。`api/store/terminal-intent.php` が `PAYMENT_NOT_AVAILABLE` を返すのは正常。

### 4.11.4 店舗決済端末の障害

POSLA外の決済端末・QRアプリ側の障害。店舗の決済会社へ確認する。POSLA側では、決済完了が確認できた取引だけを記録する。
...
A. 通常レジでは利用可能。店舗のカード端末・QR決済で支払いを完了し、POSLAには `payment_method='card'` / `payment_method='qr'` で記録のみ残す。POSLA決済（セルフ会計・テイクアウト）は Stripe Connect を使う。

**Q4. テナントの決済が突然失敗する**
A. まず通常レジかPOSLA決済かを切り分ける。通常レジのカード/QRなら店舗契約の決済端末・QRアプリ側を確認。セルフ会計・テイクアウトなら `connect_onboarding_complete` / `charges_enabled` / Stripe Dashboard を確認。
...
### E. Stripe Terminal / S700（履歴）

**Q21. S700 はいくらですか？**
A. 現行方針では通常レジの Stripe Terminal 連携を新規利用しない。店舗が外部端末として Stripe Reader を独自利用する場合でも、POSLAは決済実行せず記録のみ行う。

**Q22. S700 を複数台繋げますか？**
A. POSLAのレジアプリには接続しない。店舗側の外部決済端末として使う場合はStripe側の運用に従う。

**Q23. Connection Token の有効期限は？**
A. 現行レジアプリは Connection Token を取得しない。`api/connect/terminal-token.php` は履歴互換の扱い。

**Q24. Pattern A と B で S700 の使い方は変わりますか？**
A. 現行レジアプリではどちらのパターンでも S700 連携を行わない。セルフ会計・テイクアウトのみ Stripe Connect 経路を使う。
...
A. POSLA通常レジのテスト対象外。決済テストは、通常レジの外部決済記録と、セルフ会計・テイクアウトのStripe Connect Checkoutに分けて実施する。

---

## X.X 更新履歴
- **2026-04-29 (通常レジ支払い詳細)**: `payment_method_detail` を追加し、通常レジのカード種別 / QRブランド / 電子マネー種別を payments / orders に記録。レジ分析・取引履歴・領収書に反映。
- **2026-04-28 (決済方針統一)**: 通常レジは店舗既存決済の記録専用に変更。`process-payment.php` は `cash/card/qr` のみ受け付け、`payment_entry_mode='gateway'` と Stripe Terminal は停止。セルフ会計・テイクアウトのStripe Connect経路は維持。
