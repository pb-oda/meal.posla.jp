...
keywords: [監視, 異常検知, Google Chat, OP, runner, UptimeRobot, cron]
...
POSLA 本番運用で、サーバーやアプリケーションの異常を検知して **Google Chat + OP + メール** へ連携する仕組み。
Slack Webhook は legacy fallback として残しますが、canonical は Google Chat と `op.posla.jp` 連携です。
...
- 異常時: monitor_events に記録 + Google Chat 通知 + OP alert ingest + (連続 3 件で) 運営通知先（support@posla.jp）
...
### Step 2: Google Chat Webhook 作成

1. Google Chat の通知先スペースを作成
2. スペースのアプリと統合 → Webhook を追加
3. Webhook URL をコピー
4. POSLA 管理画面 → API 設定 → **Google Chat Webhook URL** に貼り付け (or SQL 直接):

```sql
UPDATE posla_settings SET setting_value = 'https://chat.googleapis.com/...'
WHERE setting_key = 'google_chat_webhook_url';
```

Slack を併用する場合のみ `slack_webhook_url` を設定します。
...
| 項目 | 検知条件 | 通知 |
|---|---|---|
| PHP Fatal / Parse / Warning | 直近 5 分の PHP error log | Google Chat + OP direct alert |
| Stripe Webhook 失敗 | subscription_events.error_message 直近 5 分 | Google Chat + OP direct alert |
...
| cron ラグ | last_heartbeat から 15 分以上経過 | 外部運用基盤 / 外部 uptime → 運営通知先 |
| サーバーダウン | ping エンドポイント応答なし | 外部運用基盤 / 外部 uptime → 運営通知先 |
...
resolved にしてよいのは、原因確認、設定修正、コード修正、またはテスト由来であることが確認できた後だけです。
未解決のまま残すと、OP の snapshot で `unresolved_monitor_events` として検出されます。

詳細は [14. OP / runner 連携運用](./13-op-runner-integration.md#147-monitor_event-の扱い) を参照してください。
...
1. Google Chat または OP Alert で通知受信
...
- `slack_webhook_url` — Slack Incoming Webhook URL (legacy fallback)
...
POSLA の監視は、POSLA本体だけで完結させません。POSLA が落ちた時に自分自身を監視できないため、外部の OP が正になります。
...
OP画面の `POSLA alerts同期` は過去分確認なので Chat 通知しません。

## 7.9 monitor-events.php API 詳細
...
|---|---|---|
| `info` | 情報のみ | DB のみ記録 |
| `warning` | 注意（1 件で対応必須でない） | OP記録、必要に応じてGoogle Chat |
| `error` | エラー（即時対応推奨） | Google Chat + OP direct alert + メール（連続 3 件以上） |
| `critical` | 重大障害 | Google Chat + OP direct alert + メール即時 |
...
【小田さんが今やること】
OPで調査ID inv-... を確認し、対象cellの復旧判断をしてください。

【理由】
5xx、監視イベント、Tier 0、またはcron遅延に該当するため人間確認が必要。
```
...
【小田さんが今やること】
顧客に実際の画面、発生時刻、支払い方法、スクリーンショットを確認してください。

【理由】
お問い合わせコードと申告内容が一致していないため、コード修正前に事実確認が必要です。
```
...
### 7.11.1 監視項目（monitor-health.php、5 分毎）
...
### 7.11.2 外形 ping（外部運用基盤側）

外部運用基盤が `/api/monitor/ping.php` を 5 分毎に叩く。補助として UptimeRobot や cron-job.org も同じ endpoint を叩けます。POSLA 管理画面自身は、POSLA停止時に表示できないため、外形監視の主体にしません。
...
| DB スロークエリ | 自動 EXPLAIN → 改善提案を Google Chat / OP 通知 |
