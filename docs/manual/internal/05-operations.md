...
**管理 UI:** POSLA 管理画面（`https://meal.posla.jp/posla-admin/dashboard.html` または `http://127.0.0.1:8081/posla-admin/dashboard.html`）→「API設定」タブ
...
| reverse proxy 設定 | 必須 | `meal.posla.jp -> app:80` | 本番サーバ側で管理 |
...
| 監視 / 外形監視 | UptimeRobot / cron-job.org / Better Uptime | `https://meal.posla.jp/api/monitor/ping.php`、必要なら HTTP cron secret | 監視 URL と通知先を新環境へ切替 |
...
- **本番（meal.posla.jp）**: 常に HTTPS なので `secure=true` で自動付与
...
| 10 | 本番 URL の確認 | `meal.posla.jp` | ドメインが変わる場合は `docker/env/app.env` も同時更新 |
...
| # | 確認 | コマンド/操作 | 期待結果 |
|---|---|---|---|
| 1 | サーバー応答 | `curl https://meal.posla.jp/api/monitor/ping.php` | `{"ok":true,...}` |
...
curl -H "X-POSLA-CRON-SECRET: xxxxxxxxxxxxxxxxxxxx" \
     https://meal.posla.jp/api/cron/monitor-health.php
```
...
- API 単体性能チェック: `curl https://meal.posla.jp/api/store/revisions.php?store_id=X&channels=kds_orders` (要認証 cookie) のレスポンス時間が安定して 50ms 以上かかるなら見直し対象
...
2. `api/store/device-registration-token.php` がプレーントークンを生成し、`setupUrl`（例: `https://meal.posla.jp/admin/device-setup.html?token=xxxx`）を返す
...
### Q1. 本番 URL は？
A. 現行の正本想定は **`meal.posla.jp`** です。ドメインを変更する場合は `docker/env/app.env` の `POSLA_APP_BASE_URL` / `POSLA_ALLOWED_ORIGINS` / `POSLA_ALLOWED_HOSTS` を同時更新してください。
...
A. `https://meal.posla.jp/api/monitor/ping.php`（200 + `{"ok":true}`）+ 該当変更画面の表示。
