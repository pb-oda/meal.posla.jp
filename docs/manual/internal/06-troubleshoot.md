...
keywords: [troubleshoot, 運営, POSLA, error_log, monitor, audit_log, docker]
...
::: warning 現在の正本はコンテナ運用です
本章で最優先で使う確認手順は `docker compose logs php`、`docker compose exec -T db mysql ...`、`curl http://127.0.0.1:8081/api/monitor/ping.php` です。
残っている共有サーバ・Sakura・sandbox 表記は履歴資料として扱ってください。
:::
...
4. **`SESSION_REVOKED` 401 が返る場合**（POSLA管理者のみ）: パスワード変更後の旧セッション。再ログインで解消。詳細は [§5.3.7 ②](./05-operations.md#-posla管理者セッション失効-session_revoked-401) 参照
5. **`RATE_LIMITED` 429 が返る場合**（POSLA管理者ログイン）: 5 分以内に 10 回失敗でロックアウト。5 分待機または `/tmp/posla_rate_limit/<md5>.json` 削除で解除。詳細は [§5.3.7 ③](./05-operations.md#-posla管理者ログインの-rate-limit失敗回数ベース) 参照
...
2. 該当機能の API を reverse proxy / Web サーバ設定で 503 Service Unavailable に切替
...
6. `cd docs/manual && npm run build:tenant && npm run build:internal` でドキュメントビルド
7. `api/lib/error-codes.php` / `docs/manual/{tenant,internal}/99-error-catalog.md` をデプロイ
...
- **サポートガイド / 運用補助センター の静的 JSON** (`tenant-supportdesk.json` / `internal-supportdesk.json` / `internal-ops-center.json`) と **エラーカタログ** (`99-error-catalog.md`) が E 番号で引かれる
...
- 表示 (Markdown / サポートガイド / エラーカタログ) は「該当コード | A&lt;br&gt;B」と複数併記されるためユーザー側の混乱は実害なし
...
```bash
# 1. maintenance.html を <deploy-root>/public/ に配置
# 2. reverse proxy または Web サーバ設定で /api/monitor/ping.php 以外を maintenance.html へ退避
# 3. docker compose up -d --build
# 4. 復旧時は maintenance 用の設定を戻して再配備
```
...
```
1. インフラ事業者のステータスページを確認
2. reverse proxy / app / db の各層が到達可能か確認
3. app / db ともに応答不能 → インフラ全体障害として扱う
```
...
| 症状 | 確認 | 対応 |
|---|---|---|
| Web は表示されるが DB エラー | `docker compose exec -T db mysql ...` で接続テスト | DB 障害の可能性、DB コンテナ / volume を確認 |
| コンテナ接続不能、Web は OK | reverse proxy / ホストを確認 | インフラ担当へ連絡 |
| メール送信のみ失敗 | メール基盤 / SPF / 外部 API を確認 | 配信基盤設定を確認 |
| 全部止まっている | コンテナ / reverse proxy / ホストとも異常 | インフラ全体障害、復旧待ち |
...
【原因】
インフラ事業者側の基盤障害（詳細は利用中の事業者ステータスページを参照）
...
| 予約メール未着 | メール基盤 / 迷惑メール / 設定 OFF | `reservation_notifications_log` 確認 | 6.2.6 |
| PIN 要求 | CB1d 仕様 | 仕様説明 | 6.1, 6.9.5 |
| AI 動かない | API キー未設定 / Gemini 障害 | `posla_settings.gemini_api_key` | 6.1, 6.11.4 |
| サーバー全体ダウン | インフラ障害 / DB 障害 | ping.php / mysql 接続テスト | 6.15 |
...
### Q8. お客様から E 番号を聞かれた
A. **POSLA管理画面の「サポート」タブ → エラーコード入力**が最速（2026-04-23 以降、非AI 静的データベースから即引き）。代表コードに無い場合は `docs/manual/tenant/99-error-catalog.md` を grep。
...
### Q14. 予約メールが届かないと言われた
A. 6.2.6 フロー。`reservation_notifications_log` で送信状態確認。メール基盤失敗または迷惑メール振り分けが原因の大半です。
...
### Q19. テスト DB を作りたい
A. 別の Docker DB か一時 MySQL を作成して `schema.sql` + 全 migration を流します。本番に影響を与えずにリストアテスト・マイグレーション dry-run が可能です。
