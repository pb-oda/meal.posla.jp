...
1. Safari で対象 URL を開く (例: `https://<production-domain>/admin/dashboard.html`)
2. 共有ボタン → 「ホーム画面に追加」
...
- [ ] KDS 起動オーバーレイで「KDSを開始」を 1 回押したか
- [ ] KDS の「開店前チェック」で通信、自動更新、通知音、マイク、画面維持、ステーションを確認したか
- [ ] KDS 端末状態帯が赤ではないか
- [ ] 必要に応じて KDS の「着手時間」を確認したか
...
1. `docker/env/app.env` の `POSLA_APP_BASE_URL` が `https://<production-domain>` になっていること
...
| 画面/関数 | 具体的ガード |
|---|---|
| KDS `KdsRenderer.handleAction / handleItemAction / advancePhase` | 関数冒頭で `_guardOfflineOrStale()` → true なら `Promise.reject(new Error('offline_or_stale'))` を返し、楽観更新すらしない |
...
KDSは `public/kds/index.html` の `#kds-terminal-status` に端末状態を常時表示する。

| 項目 | 取得元 | 表示/判定 |
|---|---|---|
| 最終更新 | `_lastPollingAt` | `PollingDataSource.start()` 成功時に更新 |
| 遅延 | `Date.now() - _lastPollingAt` | 10秒超で黄、20秒超で赤 |
| stale/live | `OfflineStateBanner.isStale()` | staleなら赤 |
| 音声成功 | `localStorage: mt_kds_voice_diag_v1.lastSuccessAt` | 音声コマンドの最後の成功 |
| 通信異常 | `_lastPollingError` | エラー文字列があれば赤 |

目的は「開きっぱなし端末が止まっていることに厨房が早く気づく」こと。監査用途ではなく、個人スタッフの識別にも使わない。

#### KDS着手時間設定 (2026-04-29)

予約/テイクアウトの事前表示閾値は、固定20分から端末設定へ変更した。

| 構成 | ファイル/保存先 | 内容 |
|---|---|---|
| UI | `public/kds/index.html` / `#kds-prep-lead-btn` | ヘッダーの `着手時間` ボタンから設定 |
| 保存 | `localStorage: mt_kds_prep_lead_v1` | `takeoutMin`, `reservationMin`, `categories`。`defaultMin` は古い端末設定との互換値 |
| API | `api/kds/orders.php` | 品目に `category_id` / `category_name` を付与 |
| 適用 | `public/kds/js/kds-renderer.js` | `KdsRenderer.setPrepLeadConfig()` で受け取り、予約/テイクアウトのprep判定に使う |

同じ注文に複数カテゴリがある場合は、未完了品目の中で最も長いカテゴリ別着手時間を使う。カテゴリ別が空欄の品目はテイクアウト/予約の設定に従う。DB migrationは追加していないため、店舗全体の統一設定ではなく端末ごとの設定である。
