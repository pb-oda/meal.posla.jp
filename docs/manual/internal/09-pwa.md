...
| 保存 | `localStorage: mt_kds_prep_lead_v1` | `defaultMin`, `takeoutMin`, `reservationMin`, `categories` |
...
同じ注文に複数カテゴリがある場合は、未完了品目の中で最も長いカテゴリ別着手時間を使う。DB migrationは追加していないため、店舗全体の統一設定ではなく端末ごとの設定である。
