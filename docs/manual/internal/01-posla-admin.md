...
| 現場 (customer / kds / handy) | - | AI業務機能は継続: AIウェイター / 予約AI解析 / AIシェフ / SNS生成 / 需要予測 |
...
| 環境 | URL | Basic 認証 |
|---|---|---|
| 本番 | `https://<production-domain>/posla-admin/` | あり（社内共有 ID/PW） |
...
**実行例（curl）：**
```bash
curl -X POST https://<production-domain>/api/posla/setup.php \
...
1. ブラウザで `https://<production-domain>/posla-admin/` または `http://127.0.0.1:8081/posla-admin/` にアクセスします
...
**実行例：**
```bash
curl -X POST https://<production-domain>/api/posla/tenants.php \
  -H "Cookie: PHPSESSID=..." \
  -H "Content-Type: application/json" \
...
| 現場 AI (AIウェイター/予約AI解析/AIシェフ) が 503 | gemini_api_key 未設定 (POSLA管理画面のサポート/運用補助タブは非AIのため本エラーの影響なし) | `SELECT setting_value FROM posla_settings WHERE setting_key='gemini_api_key'` | API 設定タブで登録 |
