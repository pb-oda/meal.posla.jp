/**
 * POSLA Admin (店舗管理) Service Worker
 *
...
(function () {
  ...
  self.addEventListener('install', function (event) {
    ...
    //
    // PWA Phase 1: 自動 skipWaiting を撤廃。
    //   旧実装: install 内で self.skipWaiting() を即発火 → 新 SW が waiting に留まらず即 activate
    //          → PWAUpdateManager の updatefound 検知が正しく動かない (waiting フェーズが消失)
    //   新実装: skipWaiting は postMessage('SKIP_WAITING') を待つ
    //          → PWAUpdateManager の「更新する」ボタンで明示発火
    //   初回 install (controller なし) は仕様により自動的に activate されるため、新規ユーザーは影響なし
    //   既存タブが開いている 2 回目以降のデプロイでは waiting に留まり、更新バナー経由で切替
  });
  ...
  //   payload は JSON.parse する。不正でも SW が落ちないよう try/catch。
  //   ペイロード構造: { title, body, icon?, badge?, tag?, url? }
  ...
  // ---- notificationclick: 通知タップで対象画面を開く ----
